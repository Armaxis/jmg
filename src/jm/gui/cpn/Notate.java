...
public class Notate extends Frame implements 
	...
	public void appendScore(Score score2) {
		
		
		Score dupl = new Score();
		for (int k = 0; k < score2.size(); k++)
		{
			Part iteratorPart = score2.getPart(k); 
		...
		Mod.merge(score, dupl);
		
    	Part[] scoreParts = score.getPartArray();
...
//    		final int PEACK = 12;
//    		Vector<Note> notes = scoreParts[i].getPhrase(0).getNoteList();
//    		if(notes.size() > 40)
//    		{
//    			for (int j = 0; j < PEACK; j ++)
//    			{
//    				notes.remove(0);
//    			}
//    		}
